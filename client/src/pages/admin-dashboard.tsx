...
import { Loader2, LogOut, Coffee, Users, ShoppingBasket, Check, Clock, AlertTriangle } from "lucide-react";
              ...
              {tables.map((table) => (
                <Card key={table.id} className={table.status === "occupied" ? "border-beach-yellow" : ""}>
                  ...
                  <CardContent className="p-4">
                    {table.status === "occupied" && ordersByTable[table.id] ? (
                      <div className="space-y-4">
                        <ScrollArea className="h-48">
                          {ordersByTable[table.id].map((order) => (
                            <div key={order.id} className="mb-4 p-3 border rounded-md">
                              <div className="flex justify-between items-center mb-2">
                                <div className="font-semibold">Pedido #{order.id?.substring(0, 6)}</div>
                              ...
                              <div className="mt-3 flex justify-between gap-2">
                                {order.status === "aguardando" && (
                                  <>
                                    <Button
                                      size="sm"
                                      className="flex-1 bg-beach-orange hover:bg-orange-600 text-white"
